...
import AddBenifit from "./addbenifit";

export function useTable3({ attributes, tableType, ids, data,onResponse }) {
  const { showAlert } = useAlert();
  const [selected, setSelected] = useState([]);

  const [openBenifitModal, setOpenBenifitModal] = useState(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await deleteAllBenifits(ids , { ids: selected });
  ...
  return {
    tableUI3: (
      <>
        <AddBenifit
          open={openBenifitModal}
          setOpen={setOpenBenifitModal}
          Modeltype={modeltype}
          Modeldata={modelData}
